...
# Lấy dữ liệu từ Fact
sql_query = "select * from Fact_Doanh_so_TOA where Ngay_mo_lop >= DATEADD(MONTH, -5, GETDATE()) "
df_des = transformer.fetch_from_sql_server(sql_query)

# Đồng bộ dữ liệu 
syncer = SCDType1SyncFact(df_src, df_des, key_column='Id',table_name="Fact_Doanh_so_TOA")
syncer.sync()