...
df=pd.read_csv("~/DWH_Cole_Project/data_tmp/Doanh_thu_TOA.csv")
# Chuẩn hoá dữ liệu
...
df_khong_combo = df_khong_combo.groupby(
    [col for col in df_khong_combo.columns if col != 'Doanh_thu']
).agg({'Doanh_thu': 'sum'}).reset_index()
...
def xu_ly_nhom(gr):
    ...
    # Kiểm tra xem các giá trị Doanh_thu có bằng nhau không
    if gr['Doanh_thu'].nunique() == 1:
        # Nếu bằng nhau: chia giá trị đó cho số bản ghi
        gia_tri_moi = gr['Doanh_thu'].iloc[0] / len(gr)
    else:
        # Nếu khác nhau: cộng tổng và chia đều
        gia_tri_moi = gr['Doanh_thu'].sum() / len(gr)

    # Gán lại cột Doanh_thu bằng giá trị mới
    gr['Doanh_thu'] = gia_tri_moi
    return gr
...
df_combo = df_combo.groupby(
    [col for col in df_combo.columns if col != 'Doanh_thu']
).agg({'Doanh_thu': 'sum'}).reset_index()
...
df.to_csv("~/DWH_Cole_Project/data_result/Doanh_thu_TOA_transformed.csv",index=False)
